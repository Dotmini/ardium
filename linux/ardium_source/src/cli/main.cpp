...
// --- Command: VERSION ---
void cmd_version() {
    std::cout << "Ardium CLI v2.5.5 (Titan Engine v5.0)" << std::endl;
    std::cout << "Build Date: " << __DATE__ << std::endl;
    std::cout << "Maintainer: Tirawat Nantamas (Dotmini Software)" << std::endl;
}

// --- Main Entry ---
int main(int argc, char* argv[]) {
    // print_banner(); // Only print banner on start or help, not every command? 
    // User asked "when arc showdetail", implies running 'arc' prints details.
    
    if (argc < 2) {
        print_banner();
        std::cout << "\nUsage: arc <command> [args]" << std::endl;
        std::cout << "Commands:" << std::endl;
        std::cout << "  run <file.ar>    Run a script using Titan Engine" << std::endl;
        std::cout << "  build <file.ar>  Compile a script to executable" << std::endl;
        std::cout << "  dev <file.ar>    Run in watch mode" << std::endl;
        std::cout << "  test             Run tests" << std::endl;
        std::cout << "  doctor           Check environment" << std::endl;
    ...
    if (cmd == "new" && argc > 2) {
        ...
        print_banner(); // User likes the details
    } else {
