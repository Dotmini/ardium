...
%token FN LET VAR IF ELSE LOOP RETURN EXTERN PRINT EXPORT TEST IMPORT INTERRUPT
...
top_level:
  | attrs = decorators; FN; name = ID; LPAREN; args = func_args_def; RPAREN; LBRACE; body = stmts; RBRACE
  ...
  | attrs = decorators; CLASS; name = ID; p = option(parent_decl); LBRACE; body = stmts; RBRACE
    { ClassDef(name, p, body, attrs) }
  | STRUCT; name = ID; LBRACE; opt_newline; fields = struct_field_list; RBRACE
...
stmt_base:
  ...
  | IF; cond = expr; LBRACE; ts = stmts; RBRACE; es = option(else_block)
      { let es_body = match es with Some b -> b | None -> [] in If(cond, ts, es_body) }
  | IF; LPAREN; cond = expr; RPAREN; LBRACE; ts = stmts; RBRACE; es = option(else_block)
      { let es_body = match es with Some b -> b | None -> [] in If(cond, ts, es_body) }
  | LOOP; cond = expr; LBRACE; b = stmts; RBRACE { While(cond, b) }
  | LOOP; LPAREN; cond = expr; RPAREN; LBRACE; b = stmts; RBRACE { While(cond, b) }
...
else_block:
  | ELSE; LBRACE; body = stmts; RBRACE { body }
