...
module Builtins = struct
  ...
  let io_functions = ["print"; "show"; "println"; "printf"]
  ...
  let get_format_string ctx ty =
    let fmt_str =
      if ty = Types.double_type ctx then "%.6f\n"
...
module Expr = struct
  ...
  and codegen_io_call ctx _fname args_val =
    if Array.length args_val = 0 then
      const_int (Types.i64_type ctx) 0
    else begin
      let printf_func = Builtins.get_or_declare_printf ctx in
      let arg = args_val.(0) in
      let arg_type = type_of arg in
      let fmt = Builtins.get_format_string ctx arg_type in
      let printf_ty = var_arg_function_type (Types.i32_type ctx) [| Types.i8_ptr_type ctx |] in
      let call = build_call printf_ty printf_func [| fmt; arg |] "printf_call" ctx.Context.builder in
      build_sext call (Types.i64_type ctx) "printf_ret" ctx.Context.builder
    end
...
module Proto = struct
  let codegen ctx name args ret_type is_extern =
    ...
    let arg_types_list = 
        List.map (fun _ -> 
            if is_math then Types.double_type ctx else Types.i64_type ctx
        ) args 
    in
...
let codegen_toplevel ctx = function
  | Ast.Extern (name, args, ret_type, _is_var, _decs) ->
      ...
      let args_fmt = List.map (fun (n, t) -> (n, Some t)) args in
